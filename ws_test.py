...
class CrashWebsocketTester:
    ...
    async def connect(self) -> bool:
        ...
        while retry_count < max_retries:
            ...
            if retry_count < max_retries:
                logging.info(f"Retrying connection in {reconnect_delay} seconds... (Attempt {retry_count}/{max_retries})")
        ...
        try:
            ...
            if event_type == 'crash':
                if self.last_crash_time:
                    ...
                    if interval > 45.0:
                        logging.warning(f"EXTENDED INTERVAL DETECTED: {interval:.2f}s - Possible high crash incoming")
                    
                    # Check for specific timing patterns
                    if 82.0 <= interval <= 96.0:
                        logging.warning(f"EXTREME CRASH PATTERN: {interval:.2f}s interval detected - Possible >80x crash incoming")
                    elif 45.0 <= interval <= 67.0:
                        logging.warning(f"HIGH CRASH PATTERN: {interval:.2f}s interval detected - Possible 8-15x crash incoming")
                ...
                if 'multiplier' in event_data:
                    ...
                    if len(self.multiplier_sequence) >= 3:
                        ...
                        if all(x < 2.0 for x in last_three):
                            logging.warning("THREE LOW CRASHES DETECTED - High crash probability in next round")
                        ...
                        if len(self.multiplier_sequence) >= 4:
                            if all(last_three[i] < last_three[i+1] for i in range(len(last_three)-1)):
                                logging.warning("PROGRESSIVE INCREASE DETECTED - Possible extreme crash incoming")
            ...
            if 'players' in str(event_data):
                try:
                    ...
                    if player_count > 3:
                        logging.warning(f"HIGH PLAYER COUNT: {player_count} - Increased crash probability")
                    elif player_count <= 2:
                        logging.info(f"LOW PLAYER COUNT: {player_count} - Possible low crash incoming")
    ...
    def analyze_sequence(self):
        ...
        if all(x < 2.0 for x in last_four[:-1]) and last_four[-1] > 5.0:
            logging.warning("PATTERN DETECTED: Three low crashes followed by high crash")
