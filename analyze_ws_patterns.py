...
def analyze_timing_patterns(timing_file):
    ...
    if len(auth_timing) >= 2:
        auth_gaps = [
            auth_timing[i + 1] - auth_timing[i] for i in range(len(auth_timing) - 1)
        ]
    ...
    if len(undefined_timing) >= 2:
        undefined_gaps = [
            undefined_timing[i + 1] - undefined_timing[i]
            for i in range(len(undefined_timing) - 1)
        ]
...
def analyze_message_sequences(log_file):
    ...
    if sequences:
        ...
        for pattern, count in sorted(
            ...
            if count > 1:
                logging.warning(
                    f"Found repeated message pattern ({count} times): {pattern}"
                )
