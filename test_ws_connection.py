                ...
                if len(data) >= 2:
                    ...
                    if event_name == "crash.tick":
                        if len(self.crash_times) > 0:
                            ...
                            if len(self.intervals) >= 3:
                                ...
                                if abs(recent_intervals[-1] - avg_interval) > 5:
                                    logging.warning(f"Unusual interval detected: {recent_intervals[-1]:.2f}s vs avg {avg_interval:.2f}s")
                        ...
                        if "undefined" in str(event_data):
                            self.suspicious_sequences.append((current_time, "undefined_state"))
                            self.analyze_suspicious_sequence()
                            
                        # Track 401 errors
                        if "401" in str(event_data):
                            self.suspicious_sequences.append((current_time, "auth_error"))
                            self.analyze_suspicious_sequence()
        except Exception as e:
