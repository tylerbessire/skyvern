...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ws_pattern_test.log'),
        ...
        try:
            ...
            if "crash" in message.lower():
                ...
                if len(self.crash_times) >= 2:
                    ...
                    logging.info(f"Crash interval: {interval:.2f}s (avg: {avg_interval:.2f}s)")
                    
                    # Check for consistent patterns
                    if len(self.intervals) >= 3:
                        std_dev = (sum((x - avg_interval) ** 2 for x in self.intervals[-3:]) / 3) ** 0.5
                        if std_dev < 0.5:  # Very consistent timing
                            logging.warning(f"Highly consistent crash intervals detected: {avg_interval:.2f}s ±{std_dev:.2f}s")
