...
def analyze_websocket_patterns() -> None:
    ...
    for line in console_output.split("\n"):
        if any(
            ...
            try:
                ...
                console_logs.append(
                    {"timestamp": timestamp, "type": msg_type, "message": line.strip()}
                )
    ...
    undefined_states = console_df[
        console_df["message"].str.contains("undefined", na=False)
    ]
    crash_states = console_df[
        console_df["message"].str.contains("crash|0.00x", na=False)
    ]
    ...
    if len(undefined_states) > 0 and len(crash_states) > 0:
        ...
        for idx, undef in undefined_states.iterrows():
            next_crash = (
                crash_states[crash_states["timestamp"] > undef["timestamp"]].iloc[0]
                if len(crash_states[crash_states["timestamp"] > undef["timestamp"]]) > 0
                else None
            )
            if next_crash is not None:
                time_to_crash = (
                    next_crash["timestamp"] - undef["timestamp"]
                ).total_seconds()
    ...
    if len(auth_errors) > 0:
        ...
        for idx, error in auth_errors.iterrows():
            next_state = (
                console_df[console_df["timestamp"] > error["timestamp"]].iloc[0]
                if len(console_df[console_df["timestamp"] > error["timestamp"]]) > 0
                else None
            )
            if next_state is not None:
                time_to_next = (
                    next_state["timestamp"] - error["timestamp"]
                ).total_seconds()
    ...
    for idx, row in df.iterrows():
        ...
        if len(relevant_messages) > 0:
            ...
            print(f"\nCrash at {crash_time}:")
            print(
                f"Crash value: {crash_value:.2f}x {'(HIGH)' if is_high_crash else ''}"
            )
            print("Preceding messages:")
            for _, msg in relevant_messages.iterrows():
                time_diff = crash_time - msg["timestamp"]
                print(
                    f"- {msg['type']} at {time_diff.total_seconds():.2f}s before crash: {msg['message']}"
                )
    ...
    for idx, crash in high_crashes.iterrows():
        ...
        if len(pre_crash_messages) > 0:
            ...
            for _, msg in pre_crash_messages.iterrows():
                ...
                print(
                    f"- {time_diff.total_seconds():.2f}s before: {msg['type']} - {msg['message']}"
                )
