...
def upgrade() -> None:
    ...
    op.add_column(
        "workflows", sa.Column("persist_browser_session", sa.Boolean(), nullable=True)
    )
    op.execute(
        "UPDATE workflows SET persist_browser_session = False WHERE persist_browser_session IS NULL"
    )
